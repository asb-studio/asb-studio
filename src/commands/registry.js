...
const ICONS = {
  ...
  word: 'M4 4h16v16H4zM8 9l2 6 2-4 2 4 2-6',
  ...
  record: 'M8 3h8a2 2 0 0 1 2 2v16l-6-3-6 3V5a2 2 0 0 1 2-2zM9 8h6M9 12h4',
   ...
   pullquote: 'M7 7h4v5H9a2 2 0 0 1-2-2zM17 7h-4v5h2a2 2 0 0 0 2-2zM5 17h14',
   colophon: 'M4 6h10M4 10h8M4 14h6M4 18h4',
   editornote: 'M4 20l1.2-4.2L16.5 4.5a2.1 2.1 0 0 1 3 3L8.2 18.8zM13.5 7.5l3 3',
   smalltext: 'M4 18L8 7l4 11M5.6 14.5h4.8M15 18l2-5 2 5M15.8 16h2.4',
   scenebreak: 'M6 12h.01M12 12h.01M18 12h.01',
   callout: 'M5 4h14a1 1 0 0 1 1 1v11l-4 4H5a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1zM12 8v4M12 15h.01',
   details: 'M4 5h16v14H4zM8 10l4 4 4-4',
   marker: 'M4 20h7M6.5 16l9-9 3.5 3.5-9 9H6.5z',
};
...
export const COMMANDS = [
  ...
  { id: 'push-save', menu: 'file', label: 'ذخیره در فضای مشترک', icon: 'cloudsave',
    ...
    { id: 'import-docx', menu: 'file', label: 'باز کردن فایل ورد', icon: 'word',
    tip: 'تبدیل یک فایل .docx به مارک‌دان',
    run: (ctx) => ctx.importDocx() },
  ...
  /* The four typographic quotes. No `key` here on purpose: the numpad
     binding lives in main.js matched on e.code, because a registry key like
     Mod-Alt-1 would also catch the TOP-ROW digit - and that row belongs to
     tab switching. The menu keeps them reachable with or without a numpad. */
  { id: 'quote-single-open', menu: 'insert', label: 'نقل‌قول تکی باز (‘)', textIcon: '‘',
    tip: 'نقل‌قول تکی باز (Ctrl+Alt+1)', run: (ctx) => ctx.editor.replaceSelection('\u2018') },
  { id: 'quote-single-close', menu: 'insert', label: 'نقل‌قول تکی بسته (’)', textIcon: '’',
    tip: 'نقل‌قول تکی بسته (Ctrl+Alt+2)', run: (ctx) => ctx.editor.replaceSelection('\u2019') },
  { id: 'quote-double-open', menu: 'insert', label: 'نقل‌قول دوتایی باز (“)', textIcon: '“',
    tip: 'نقل‌قول دوتایی باز (Ctrl+Alt+3)', run: (ctx) => ctx.editor.replaceSelection('\u201C') },
  { id: 'quote-double-close', menu: 'insert', label: 'نقل‌قول دوتایی بسته (”)', textIcon: '”',
    tip: 'نقل‌قول دوتایی بسته (Ctrl+Alt+4)', run: (ctx) => ctx.editor.replaceSelection('\u201D') },
  ...
  /* The excerpt wraps the author's SELECTION: هر چند بند که خواستی. START
     lands above it, END below it - no sample text, nothing else moves. */
  { id: 'excerpt', menu: 'insert', label: 'بریده‌ی نمایشی', icon: 'excerpt',
    tip: 'متن بریده را انتخاب کن و بزن — استارت و اِند دورش می‌گذارد',
    run: (ctx) => ctx.insertExcerpt() },
  ...
  { id: 'scene-break', menu: 'insert', label: 'فاصله‌ی صحنه', icon: 'scenebreak',
    tip: '* * * اُخرایی روی صفحه؛ در کتاب‌خوان فقط سفیدی. جای --- این',
    run: (ctx) => ctx.editor.insertBlock('<div class="scene-break"></div>') },
  { id: 'admonition', menu: 'insert', label: 'جعبه‌ی هشدار', icon: 'callout',
    tip: '/// note | عنوان — نه رنگ: اخرایی، آبی، قرمز',
    run: (ctx) => ctx.insertAdmonition() },
  { id: 'details', menu: 'insert', label: 'بخش تاشو', icon: 'details',
    tip: '/// details — افشای پایان و پرسش‌وپاسخ',
    run: (ctx) => ctx.insertDetails() },
  ...
  { id: 'mark', menu: 'format', group: 'text', label: 'برجسته', icon: 'marker',
    tip: 'برجسته با <mark> — سایت mark می‌شناسد', run: (ctx) => ctx.editor.wrapSelection('<mark>', '</mark>') },
  { id: 'sup', menu: 'format', group: 'text', label: 'بالانویس', textIcon: 'x²',
    tip: 'بالانویس — مارک‌دانِ سایت ^ این را ندارد', run: (ctx) => ctx.editor.wrapSelection('<sup>', '</sup>') },
  { id: 'sub', menu: 'format', group: 'text', label: 'زیرنویس', textIcon: 'x₂',
    tip: 'زیرنویس — مارک‌دانِ سایت ~ این را ندارد', run: (ctx) => ctx.editor.wrapSelection('<sub>', '</sub>') },
